...
import {createPersonaPrompt, createAvatar, joinPrompt, responsePrompt, systemPrompt, chatPrompt} from "./prompts";
import {Subscription} from "encore.dev/pubsub";
import {User} from "../../chat/chat/chat";
...
export const generateBotProfile = api(
  ...
  async ({name, prompt, provider}: {name: string, prompt:string, provider:string}): Promise<{ profile:string, avatar?:string }> => {
    let client = availableClients[provider as LLMType];
    if (!client) {
      throw new Error("Provider not available")
    }
    const profile = await client.ask({prompt: createPersonaPrompt.replace("{name}", name).replace("{prompt}", prompt)})
    if(!client.generateAvatar) {
      return {profile: profile.response}
    }

    let avatar = await client.generateAvatar({prompt: createAvatar.replace("{profile}", prompt)})
    // we store the avatar in the db, so we resize it to make in managable
    avatar.data = await sharp(Buffer.from(avatar.data, "base64")).resize(100, 100).toBuffer().then((data) => {
...
export const handleLLMTask = api(
  ...
  async (task: LLMTask): Promise<void> => {
    ...
    const usersByID = task.users.reduce((acc, user) => {
      acc[user.id] = user
      return acc
    }, {} as Record<number, User>)
    const botsByID = task.bots.reduce((acc, bot) => {
      acc[bot.id] = bot
      return acc
    }, {} as Record<number, Bot>)

    let botProfiles = task.bots.map((bot, i) => `${i}: ${bot.profile}`).join("\n");
