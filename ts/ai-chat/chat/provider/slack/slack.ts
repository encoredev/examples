...
import {ProviderChannel, ProviderMessage, ProviderUser, SendMessageRequest} from "../provider";
import {Message} from "../../chat/chat";
import {providerMessageTopic} from "../../chat/topics";
...
export const sendMessage = api(
  ...
  async ({ channel, req }: { channel: string, req: SendMessageRequest }): Promise<void> => {
...
const toProviderMessage = (msg: MessageElement, channel: string): ProviderMessage | null => {
  ...
  if(msg.subtype === "bot_message") {
    ...
    if(msg.metadata?.event_type === "bot_message") {
      const payload = msg.metadata.event_payload as {bot_id: number} | undefined;
...
export const listMessages = api(
  ...
  async ({ channel, oldest }: { channel: string, oldest?: Message}): Promise<{messages?: ProviderMessage[]}> => {
    const oldestTs = oldest ? oldest.timestamp / 1000 : undefined;
    log.debug("Listing messages", {channel, oldestTs})
    return client.conversations.history({channel, oldest: oldestTs?.toString()}).then((resp) => {
      return {messages: resp.messages?.map((m) => {
