...
import { llmTasks } from "../../llm/service/topics";
import { botEventTopic, BotEvent } from "../../llm/service/topics";
import { providerMessageTopic } from "./topics";
import { availableClients, ChatType } from "./clients";
import { LLMType as LLMClientType } from "../../llm/service/clients";
...
export const getChannelByProviderID = api(
  {
    expose: false,
    method: "GET",
    path: "/chat/provider/:provider/channels/:id",
  },
  async ({
    id,
    provider,
...
export const addBotToProviderChannel = api(
  {
    expose: false,
    method: "POST",
    path: "/chat/provider/:providerID/channels/:channelID/bot/:botID",
  },
  async ({
    channelID,
    botID,
    ...
    let channel = await getChannelByProviderID({
      id: channelID,
      provider: providerID,
    }).catch(() => {});
    ...
    await BotChannels()
      .insert({ channel_id: channel.id, bot_id: botID })
...
export const loadChannelHistory = async (channel: Channel) => {
  ...
  const messages = await chatsvc.listMessages({
    channel: channel.provider_id,
    oldest: lastMessage,
  });
  ...
  switch (event.kind) {
    ...
    case "message":
      await chatSvc.sendMessage({
        channel: channel.provider_id,
        req: { text: event.message!, bot: info },
      });
      break;
    default:
...
export const handleChatMessage = async (message: ProviderMessage) => {
  ...
  const channel = await getChannelByProviderID({
    id: message.channelID,
    provider: message.provider,
  });
  const channelBots = await BotChannels()
    .select("bot_id")
    .where("channel_id", channel.id);
  if (channelBots.length === 0) {
    return;
  }
  const bots = await botsvc.list({
    ids: channelBots.map((b) => b.bot_id).join(","),
  });
...
export const insertProviderMessages = async (
  provider: ChatType,
  ...
  for (const message of messages) {
    ...
    if (!user) {
      ...
      if (!user.bot_id) {
        const providerUser = await chatsvc.getUser({
          userID: message.user.providerID,
        });
        if (providerUser) {
    ...
    if (!channel) {
      const provChannel = await chatsvc.getChannel({
