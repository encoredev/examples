...
// Get the authenticated user's profile. Combines auth data from WorkOS
...
export const getProfile = api(
  ...
  async (): Promise<ProfileResponse> => {
    ...
    let profile = await db.queryRow<{
      ...
      FROM user_profile
      WHERE workos_user_id = ${auth.userID}
    `;

    // Create profile with defaults if it doesn't exist
    if (!profile) {
      await db.exec`
        INSERT INTO user_profile (workos_user_id, theme, notifications)
...
export const updatePreferences = api(
  ...
  async (req: UpdatePreferencesRequest): Promise<UpdatePreferencesResponse> => {
    ...
    await db.exec`
      INSERT INTO user_profile (workos_user_id, theme, notifications, bio, updated_at)
