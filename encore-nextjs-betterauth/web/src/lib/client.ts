...
export function Environment(name: string): BaseURL {
    return `https://${name}-c26fs.encr.app`
}
...
/**
 * Client is an API client for the c26fs Encore application.
 */
...
export namespace auth {
    ...
    success: boolean
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/auth/signout`, JSON.stringify(params))
...
export namespace sessions {
    ...
    export class ServiceClient {
        ...
        public async listSessions(): Promise<void> {
            await this.baseClient.callTypedAPI("GET", `/sessions`)
        }

        public async revokeSession(id: string): Promise<void> {
            await this.baseClient.callTypedAPI("DELETE", `/sessions/${encodeURIComponent(id)}`)
        }
    }
}
...
export namespace todos {
    ...
    export class ServiceClient {
        ...
        public async todos(): Promise<Response> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/todos`)
            return await resp.json() as Response
        }
    }
}
...
export namespace user {
    ...
    export class ServiceClient {
        ...
        public async currentUser(): Promise<UserProfile> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/current-user`)
            return await resp.json() as UserProfile
        }

        public async updateProfile(params: UpdateProfileRequest): Promise<UserProfile> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("PUT", `/update`, JSON.stringify(params))
            return await resp.json() as UserProfile
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (!BROWSER) {
            this.headers["User-Agent"] = "c26fs-Generated-TS-Client (Encore/v1.53.6)";
        }
