...
export const createCheckoutSession = api(
  ...
  async (
    ...
    const org = await clerk.organizations.getOrganization({ organizationId: authData.orgID });
...
export const getSubscription = api(
  ...
  async (): Promise<GetSubscriptionsResponse> => {
    ...
    if (!stripeCustomerId) {
      throw APIError.notFound("organization does not have a Stripe customer ID");
    }
