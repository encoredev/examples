...
export const createCheckoutSession = api({
  ...
  expose: true,
  auth: true
}, async (params: CreateCheckoutSessionParams): Promise<CreateCheckoutSessionResponse> => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    mode: 'subscription',
    success_url: `http://localhost:3000/dashboard/subscription?success=true&session_id={CHECKOUT_SESSION_ID}`,
